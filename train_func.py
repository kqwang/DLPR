...
def train_net_PD(net, device, loader, optimizer, loss_f, prop_dis=20, norm=False, rand_to_holo=False):
    net.train()
    train_loss = AverageMeter()
    for batch_idx, (input, gt) in enumerate(loader):
        input, gt = input.to(device), gt.to(device) # Send data to GPU
        ...
        if rand_to_holo:
            rand = torch.rand((1, 1, 256, 256)) * (1.0/30)
            input = input + rand.to(device)

        output = net(input)  # network forward
        H = propagation(output, prop_dis=prop_dis, norm=norm)
...
def train_net_CD(net, device, loader, optimizer, loss_f, prop_dis=20, norm=False, rand_to_holo=False):
    net.train()
    train_loss = AverageMeter()
    for batch_idx, (input, gt) in enumerate(loader):
        input, gt = input.to(device), gt.to(device) # Send data to GPU
